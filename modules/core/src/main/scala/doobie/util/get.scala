...
import doobie.util.invariant.{InvalidObjectMapping, InvalidValue}
...
sealed abstract class Get[A](val typeStack: NonEmptyList[Option[Type]],
  ...
  final def temap[B](
    f: A => Either[String, B]
  )(implicit sA: Show[A], evA: TypeTag[A], evB: TypeTag[B]): Get[B] =
    tmap { a =>
      f(a) match {
...
object Get extends GetInstances {
  ...
  object Basic {
    ...
    def many[A](jdbcSources: NonEmptyList[JdbcType],
                jdbcSourceSecondary: List[JdbcType],
                get: (ResultSet, Int) => A)(implicit ev: TypeTag[A]): Basic[A] =
      Basic(
    ...
    def one[A: TypeTag](jdbcSources: JdbcType,
                        jdbcSourceSecondary: List[JdbcType],
                        get: (ResultSet, Int) => A): Basic[A] =
  ...
  object Advanced {
    ...
    def other[A >: Null <: AnyRef: TypeTag](
      schemaTypes: NonEmptyList[String]
    )(implicit A: ClassTag[A]): Advanced[A] =
      many(
            ...
            case a    =>
              ...
              try A.runtimeClass.cast(a).asInstanceOf[A]
                ...
                case _: ClassCastException =>
                  throw InvalidObjectMapping(A.runtimeClass, a.getClass)
              }
          }
        }
      )
  ...
  implicit def metaProjection[A](implicit m: Meta[A]): Get[A] =
    m.get
...
trait GetInstances {
  ...
  /** @group Instances */
  implicit def ArrayTypeAsListGet[A: ClassTag: TypeTag](
    implicit ev: Get[Array[A]]
  ): Get[List[A]] =
    ev.tmap(_.toList)

  /** @group Instances */
  implicit def ArrayTypeAsVectorGet[A: ClassTag: TypeTag](
    implicit ev: Get[Array[A]]
  ): Get[Vector[A]] =
    ev.tmap(_.toVector)
